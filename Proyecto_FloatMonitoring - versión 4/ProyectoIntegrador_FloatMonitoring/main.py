...
# ─── GET /api/mediciones/exportar-csv ────────────────────────────────────────
@app.get("/api/mediciones/exportar-csv", summary="Exportar mediciones como archivo CSV")
def exportar_mediciones_csv(
    fecha_inicio:  str = Query(...,
                               description="Inicio del rango (YYYY-MM-DDTHH:MM)"),
    fecha_fin:     str = Query(...,
                               description="Fin del rango   (YYYY-MM-DDTHH:MM)"),
    session_token: Optional[str] = Cookie(default=None),
):
    """
    Genera y descarga un archivo CSV con todas las mediciones del rango
    de fechas indicado, ordenadas cronológicamente. El nombre del archivo
    incluye el rango: float_monitoring_YYYYMMDD_YYYYMMDD.csv
    """
    requerir_sesion(session_token)
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail="Formato de fecha inválido. Use YYYY-MM-DDTHH:MM."
        )
    ...
    try:
        ...
        # Construir CSV en memoria — sin crear archivos temporales en disco
        buffer = io.StringIO()
        # BOM UTF-8: hace que Excel abra el archivo con codificación correcta
        # sin necesidad del asistente de importación.
        buffer.write('\ufeff')
        # Delimitador ';' en lugar de ',': en Colombia (y toda Latinoamérica)
        # Excel/LibreOffice usan ';' porque la coma ya es separador decimal
        # (ej. 7,42 = 7.42). Con ',' todos los campos caen en la columna A.
        writer = csv.writer(buffer, delimiter=';')

        # Encabezado
        writer.writerow(["ID", "pH", "Temp. Ambiente (°C)",
                        "Temp. Agua (°C)", "Fecha y Hora"])
        ...
        nombre_archivo = (
            f"float_monitoring_"
            f"{dt_inicio.strftime('%Y%m%d')}_"
            f"{dt_fin.strftime('%Y%m%d')}.csv"
        )

        buffer.seek(0)
        logger.info(
            "CSV exportado: %d registros | %s → %s",
            len(registros), dt_inicio, dt_fin
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error al generar CSV: {e}")
    finally:
        if conexion:
            conexion.close()
