...
// Llama a GET /api/mediciones/exportar-csv con las mismas fechas del filtro
// activo. El backend devuelve un StreamingResponse con cabecera
// Content-Disposition: attachment; filename=float_monitoring_XXX.csv
// que el navegador procesa como descarga directa sin abrir nueva pestaña.
async function exportarCSV() {
    ...
    // Estado de carga en ambos botones que invocan esta función
    const btnFiltro = document.getElementById("btn-exportar-csv");
    const btnTabla = document.getElementById("btn-exportar-csv-tabla");

    const restoreBtns = () => {
        if (btnFiltro) {
            btnFiltro.disabled = false;
            btnFiltro.innerHTML = `<i class="bi bi-file-earmark-arrow-down me-1"></i>CSV`;
        }
        if (btnTabla) {
            btnTabla.disabled = false;
            btnTabla.innerHTML = `<i class="bi bi-file-earmark-arrow-down me-1"></i>Exportar CSV`;
        }
    };

    const spinnerHtml = `<span class="spinner-border spinner-border-sm me-1" role="status"
        aria-hidden="true"></span>Exportando…`;
    if (btnFiltro) { btnFiltro.disabled = true; btnFiltro.innerHTML = spinnerHtml; }
    if (btnTabla) { btnTabla.disabled = true; btnTabla.innerHTML = spinnerHtml; }

    const url = `${API_URL}/mediciones/exportar-csv` +
    ...
    try {
        ...
        if (!res.ok) {
            ...
            throw new Error(errBody.detail || `Error del servidor (HTTP ${res.status})`);
        }

        // Extraer nombre de archivo desde Content-Disposition
        const cd = res.headers.get("Content-Disposition") || "";
        const matchFilename = cd.match(/filename=([^;]+)/);
        const filename = matchFilename
            ? matchFilename[1].trim()
            : "reporte_float_monitoring.csv";

        // Convertir la respuesta en Blob y disparar la descarga
        ...
        mostrarToast(`Reporte descargado: ${filename}`, "success");

    } catch (err) {
        mostrarToast(`No se pudo exportar: ${err.message}`, "danger");
    } finally {
        restoreBtns();
    }
}
